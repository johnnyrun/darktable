...
typedef enum dt_collection_filter_comparator_t
{
  ...
  COLLECTION_FILTER_FILM_ID = 1 << 0,        // use film_id in filter 1
  COLLECTION_FILTER_ATLEAST_RATING = 1 << 1, // show all stars including and above selected star filter 2
  COLLECTION_FILTER_EQUAL_RATING = 1 << 2,   // show only selected star filter 4
  COLLECTION_FILTER_ALTERED = 1 << 3,        // show only altered images 8 
  COLLECTION_FILTER_UNALTERED = 1 << 4,      // show only unaltered images 16
  COLLECTION_FILTER_CUSTOM_COMPARE
  = 1 << 5, // use the comparator defined in the comparator field to filter stars 32
  COLLECTION_FILTER_COMPARABLE = 1 << 6      // show only unaltered images 64
